...
def extract_date_range_from_question_es(q: str) -> dict | None:
    ...
    if re.search(r'\bmes\s+pasado\b', t):
        first_this = _som(now); start = first_this - pd.tseries.offsets.MonthBegin(1); end = first_this - pd.Timedelta(days=1)
        return {"start": start.normalize(), "end": end.normalize(), "label": "mes pasado"}
    if re.search(r'\bpr[oó]ximo\s+mes\b', t):
        n = now + pd.offsets.MonthBegin(1); s = pd.Timestamp(year=int(n.year), month=int(n.month), day=1); e = (s + pd.offsets.MonthEnd(1)).normalize()
    ...
    if m:
        ...
        if pd.notna(s):
            s = s.normalize()
            return {"start": s, "end": s, "label": m.group(1)}
...
def filter_data_by_question_if_time(data: Dict[str, pd.DataFrame], question: str):
    ...
    # Convertir a pandas Timestamps y unificar a tz-naive para comparar con Series naive
    st = pd.Timestamp(rng["start"])
    en = pd.Timestamp(rng["end"])
    if getattr(st, "tz", None) is not None:
        st = st.tz_localize(None)
    if getattr(en, "tz", None) is not None:
    ...
    for h, df in (data or {}).items():
        ...
        if not date_col:
            out[h] = df
            continue
        # Parsear fechas de la hoja respetando formato día/mes/año y remover tz si aparece
