...
def facturas_por_pagar_en_dias(data, dic_idx, dias=30):
    """Retorna facturas con fecha de pago dentro de los próximos `dias` días > HOY y <= HOY + dias."""
    filas = []
    try:
        dias = int(dias)
    except Exception:
    ...
    for hoja, df in (data or {}).items():
        ...
        try:
            ...
            tmp = sub[[id_col]].copy()
            tmp["hoja"] = hoja
            tmp["fecha_pago"] = fechas.loc[sub.index]
            filas.append(tmp.rename(columns={id_col: "id"}))
        except Exception:
            continue
    import pandas as _pd
    return _pd.concat(filas, ignore_index=True) if filas else _pd.DataFrame(columns=["id","hoja","fecha_pago"])
